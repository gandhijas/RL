...
run_dir = f"results/{timestamp}_exp08_drift_corrected_diagnostic"
...
def run_episode(
    ...
    if (
        ...
        td_error = terminal_reward - q_previous
        weights[pending_action] += (
            LEARNING_RATE * td_error * pending_state
        )
...
with open(f"{run_dir}/notes.txt", "w", encoding="utf-8") as file:
    file.write("Corrected Experiment 8 drift tracking\n")
    file.write(
    ...
    file.write(f"Training episodes: 1000\n")
    file.write(f"Training restarts: 2\n")
